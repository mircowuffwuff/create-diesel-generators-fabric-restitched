...
public class OilScenes {
    public static void pumpjack(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(25);
        scene.overlay().showControls(util.vector().centerOf(4, 4, 1), Pointing.LEFT, 15).withItem(AllItems.SUPER_GLUE.asStack());
        scene.idle(25);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.GREEN, util.select().fromTo(4, 5, 9, 4, 4, 0), new AABB(4, 6, 9, 5, 4, 0), 30);
        scene.idle(15);
        scene.overlay().showControls(util.vector().centerOf(4, 4, 1), Pointing.LEFT, 15).withItem(AllItems.SUPER_GLUE.asStack());
        scene.idle(25);
        scene.overlay().showControls(util.vector().centerOf(5, 4, 3), Pointing.LEFT, 15).withItem(AllItems.SUPER_GLUE.asStack());
        scene.idle(25);
        ...
        scene.overlay().showText(50)
                ...
                .placeNearTarget();
        scene.idle(60);
        scene.overlay().showControls(util.vector().topOf(6, 4, 5), Pointing.LEFT, 15).rightClick();
        scene.idle(50);
